...
#include "AnalyzerCounterNNShellContacts.h"
...
int main(int argc, char* argv[])
{
	try{
		...
		["-o"]["--outfile"]
			   ("BFM-file to save.")
			   .required()
		| clara::Opt( [&startAge](uint64_t const m)
					   {
			if (m < 0)
			{
				return clara::ParserResult::runtimeError("Start Age must be greater than -1.");
			}
			else
			{
				startAge = m;
    ...
    taskmanager.addAnalyzer(new AnalyzerCounterNNShellContacts<Ing>(myIngredients, startAge,  "./"));
