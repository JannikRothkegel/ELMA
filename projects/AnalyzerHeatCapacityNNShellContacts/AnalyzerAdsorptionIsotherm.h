...
bool AnalyzerAdsorptionIsotherm<IngredientsType>::execute()
{
	...
	std::vector<VectorInt3> contactSites;
		...
		VectorInt3 pos;
		double numVacantVerticesInShell=0.0;
		...
		std::cout << " numVacantVerticesInShell: " << numVacantVerticesInShell << std::endl;
	...
	for(size_t n=0;n<ingredients.getMolecules().size();n++)
	{
		...
		if( monoType == 3)
		{
			...
			for(size_t contactNo=0;contactNo<contactSites.size();contactNo++)
			{
				...
				// check if surrounding places are solvent (2) or polymer (1)
				if((latticeEntry != 3))
				{
					numCoSolventInShell++;
					break;
...
void AnalyzerAdsorptionIsotherm<IngredientsType>::cleanup()
{
	...
	std::vector < std::vector<double> > tmpResults;
		...
		tmpResults.resize(12);

		for(int i = 0; i < 12; i++)
			tmpResults[i].resize(1);

		tmpResults[0][0]=ingredients.getNNInteraction(2, 3);
		...
		tmpResults[4][0]=Statistic_InternalEnergy.ReturnM1()/ingredients.getNNInteraction(2, 3);

		//tmpResults[5][0]=(Statistic_InternalEnergy.ReturnM1()/ingredients.getNNInteraction(2, 3))/(8.0*(2.0*256+1));

		//tmpResults[6][0]=(Statistic_InternalEnergy.ReturnM1()/ingredients.getNNInteraction(2, 3))/(4.0*numCoSolvent);

		tmpResults[5][0]=numCoSolvent; //tmpResults[7][0]=numCoSolvent;

		tmpResults[6][0]=Statistic_NumMonomers.ReturnM1();
		tmpResults[7][0]=Statistic_NumMonomers.ReturnM2();
		tmpResults[8][0]=Statistic_NumMonomers.ReturnM2()-Statistic_NumMonomers.ReturnM1()*Statistic_NumMonomers.ReturnM1();

		tmpResults[9][0]=Statistic_NumMonomers.ReturnM1()/numCoSolvent;


		tmpResults[10][0]=((8.0*(ingredients.getMolecules().size()))/(1.0*ingredients.getBoxX()*ingredients.getBoxY()*ingredients.getBoxZ()));

		tmpResults[11][0]=8.0*numCoSolvent/(1.0*ingredients.getBoxX()*ingredients.getBoxY()*ingredients.getBoxZ());
		...
		comment <<"File produced by analyzer AnalyzerAdsorptionIsotherm\n"
                ...
                <<"epsilon\t<U>\t<U²>\t<cV>\t<nContacts>\t<nCoSolvent>\t<nCoS In NNShell>\t<(nCoS In NNShell)²>\tvar(nCoS In NNShell)\t<(nCoS In NNShell)/nCoSolvent>\t<cAll>\t<cCoSolvent>\n";
	...
	std::string filenameRg2_Ree_b2 = filenameGeneral + "_AnalyzerHeatCapacityNNShellContactsCoSolvent.dat";
